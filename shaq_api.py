...
def get_yahoo_beta(ticker: str) -> Optional[float]:
    ...
    if ticker in _beta_cache and (now - _beta_cache_time.get(ticker, 0)) < 1800:
...
def analyze(req: AnalyzeRequest):
    ...
    # Fallback: if sector detected but not in our packs, use ETFs & Indexes
    if sector and sector not in SECTOR_PACKS:
        sector = "ETFs & Indexes"

    if not sector:
        # Last resort: try to verify ticker exists at all
        try:
            test = yf.Ticker(ticker).history(period="5d")
            if len(test) > 0:
                sector = "ETFs & Indexes"  # Valid ticker, unknown sector — use ETF pack
            else:
                raise HTTPException(status_code=400, detail=f"'{ticker}' does not appear to be a valid ticker.")
        except HTTPException:
            raise
        except Exception:
            raise HTTPException(status_code=400, detail=f"Could not find data for '{ticker}'. Check the ticker symbol.")
